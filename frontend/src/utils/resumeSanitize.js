...
function isJobMetaLine(line = '') {
  ...
  if (t.includes('|') && t.length < 200 && !/^[•-]/.test(line)) return true;
  if (/\b(Engineer|DevOps|Architect)\b/i.test(t) && DATE_RANGE_RE.test(t)) return true;
  return false;
}
...
export function sanitizeExperienceLines(text = '') {
